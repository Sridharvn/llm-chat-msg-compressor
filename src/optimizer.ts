...
export interface OptimizerOptions {
  ...
  dataRefinement?: {
    enabled?: boolean; // opt-in lossy preprocessing
    precision?: number; // float precision
    pruneEmpty?: boolean; // remove empty fields
...
export class Optimizer {
  ...
  private yamlStrat = new (require("./yamlStrategy").YamlStrategy)();
  ...
  optimize(data: any, options: OptimizerOptions = {}) {
    ...
    const { yamlEnabled = true, customDict } = options as any;

    // If a custom dict was supplied for this run, temporarily override the static DICT
    const oldDict = (AbbreviatedKeysStrategy as any).DICT;
    if (customDict) {
      (AbbreviatedKeysStrategy as any).DICT = customDict;
    }
    ...
    try {
      ...
      if (customDict) {
        ...
        (AbbreviatedKeysStrategy as any).DICT = oldDict;
      }
    }
  }
