...
describe("Compression Strategies", () => {
  ...
  describe("AbbreviatedKeysStrategy", () => {
    ...
    it("should use static DICT during decompress even when map is empty", () => {
      const dict = { auth: "a", message: "m" };
      (AbbreviatedKeysStrategy as any).DICT = dict;
      const pkg = { m: {}, d: { a: "tok", m: "hello" } };
      const restored = strategy.decompress(pkg);
  ...
  describe("UltraCompactStrategy", () => {
    ...
    it("should respect static DICT keys on decompress even if map missing", () => {
      const dict = { status: "s" };
      (AbbreviatedKeysStrategy as any).DICT = dict;
      const unsafeStrategy = new UltraCompactStrategy({ unsafe: true });
      const pkg = { m: {}, d: { s: 1 } };
      const restored = unsafeStrategy.decompress(pkg);
      expect(restored).toEqual({ status: 1 });
      delete (AbbreviatedKeysStrategy as any).DICT;
    });
  });
});
